...
**DNS 协议运行在 UDP 和 TCP 之上，使用 53 号端口。**

**DNS 解析查询时用到 UDP 协议，在主从传递区域数据库文件时，用到 TCP 协议。**
...
1. /etc/hosts 查找ip映射
2. /etc/resolv.conf 查询 DNS服务器记录或者缓存记录
3. 根 DNS 
4. 一级域名
5. 二级域名
6. 访问域名，缓存记录
...
``DNS lookup utility``    ``defult port 53(UDP)，可以是 TCP，测试 DNS 系统，不会查询 hosts 文件进行解析``
